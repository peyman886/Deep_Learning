...
def mutate_tensor(x, indices, values):
    for indie, value in zip(indices, values):
        x[indie] = value
...
def reverse_rows(x):
  ...
  """
    idx = list(range(x.shape[0] - 1, -1, -1))
    y = x[idx]
...
def batched_matrix_multiply(x, y, use_loop=True):
  ...
  """
    ...
    if use_loop:
        B = x.shape[0]
        for i in range(B-1):
            z = torch.stack((x[i] @ y[i], x[i+1] @ y[i+1]))
    return z
...
def normalize_columns(x):
  ...
  list/set/etc comprehensions); you may only use 0_basic arithmetic operations on
